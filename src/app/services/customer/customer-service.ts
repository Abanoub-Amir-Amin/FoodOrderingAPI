...
export class CustomerService {
  ...
  getCustomerById(): Observable<CustomerDto> {
    return this.http.get<CustomerDto>(
      `${this.basicUrl}Customer/ByID/${this.userid}`,
      { headers: this.headers }
    );
  }
  updateCustomer(customer: UpdateCustomerDTO): Observable<any> {
    return this.http.put(
      `${this.basicUrl}Customer/UpdateCustomer?CustomerId=${this.userid}`,
      customer,
      { headers: this.headers }
    );
  }
  ...
  getallOrders(): Observable<OrderViewDTO[]> {
    return this.http.get<OrderViewDTO[]>(
      `${this.basicUrl}Order/AllOrdersForCustomer`,
      { headers: this.headers }
    );
  }
  getorderdetails(orderId: string): Observable<OrderDetailDTO> {
    return this.http.get<OrderDetailDTO>(
      `${this.basicUrl}Order/AllOrdersForCustomer/?orderId=${orderId}`,
      {
        headers: this.headers,
      }
    );
  }
  getorderforcustomerbystatus(
    status: StatusEnum[]
  ): Observable<OrderViewDTO[]> {
    return this.http.request<OrderViewDTO[]>(
      'GET',
      `${this.basicUrl}Order/OrderForCustomerbystatus`,
      { body: { status }, headers: this.headers }
    );
  }
