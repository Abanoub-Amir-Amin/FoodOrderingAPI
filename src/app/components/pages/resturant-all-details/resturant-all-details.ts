...
import { ListOfResturant, Review } from '../../../services/list-of-resturant';
import { ReviewService } from '../../../services/review-service';

@Component({
  selector: 'app-resturant-all-details',
  standalone: true,
  imports: [MainLayoutComponent, Footer, RelativeTimePipe, Rating],
  ...
  ngOnInit(): void {
    ...
    this.restaurantService.getAllRestaurants().subscribe({
      next: (data) => {
        ...
        this.restaurant = restaurants.find(
    ...
    this.restaurantService.getAllReviews().subscribe({
      next: (data) => {
        ...
        this.reviews = (data.$values ?? []).filter(
          (r) => r.restaurantID === this.restaurant?.id
        );
      },
