...
export class OrdersManagement implements OnInit {
  ...
  loadOrders(): void {
    ...
    if (this.selectedStatusFilter === 'All') {
      url = `${this.baseUrl}/restaurant/${this.restaurantId}/orders`;
    } else {
      url = `${this.baseUrl}/restaurant/${this.restaurantId}/orders/status?status=${encodeURIComponent(this.selectedStatusFilter)}`;
    }
  ...
  private changeOrderStatus(order: any, newStatus: string): void {
    ...
    const url = `${this.baseUrl}/restaurant/${this.restaurantId}/orders/${orderId}/status`;
    const body = { orderID: orderId, status: newStatus };

    this.http.put(url, body, { headers: this.getAuthHeaders() }).subscribe({
      next: (updatedOrder) => {
        this.snackBar.open(`Order #${orderId} status updated to ${newStatus}`, 'Close', { duration: 4000 });
        this.loadOrders();
      },
      error: (err) => {
